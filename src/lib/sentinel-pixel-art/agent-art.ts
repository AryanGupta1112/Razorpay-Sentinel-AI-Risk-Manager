// Self-contained procedural art for Sentinel agent portraits and room sprites.
...
type Brow = 'flat' | 'raised' | 'soft';
type Mouth = 'neutral' | 'smile';
...
// ─── hairstyles ──────────────────────────────────────────────────────────────
interface HairArgs { length?: number; vol?: number; }
...
const HAIR_FNS = { styleFloppy, styleFrame, styleMessy, styleRecede };
type HairStyle = keyof typeof HAIR_FNS;

// ─── facial hair ─────────────────────────────────────────────────────────────
function drawMustache(buf: Buf, color: RGB): void {
...
// ─── clothing ────────────────────────────────────────────────────────────────
type Cloth = 'dressshirt' | 'blouse' | 'cardigan';
...
interface Recipe {
  ...
  brow?: Brow; mouth?: Mouth; blush?: boolean; mustache?: boolean; glasses?: boolean;
...
const RECIPES: Record<AgentCharacter, Recipe> = {
  'signal-scout': { skin: 'light', hairc: [92, 60, 34], hair: 'styleFloppy', cloth: 'dressshirt', c1: [172, 196, 224], tie: [120, 130, 150], brow: 'flat', mouth: 'smile' },
  'merchant-guard': { skin: 'light', hairc: [120, 76, 42], hair: 'styleFrame', hairargs: { length: 18, vol: 2 }, cloth: 'cardigan', c1: [236, 174, 192], c2: [244, 242, 238], brow: 'soft', mouth: 'smile', blush: true, lashes: true },
  'policy-guard': { skin: 'dark', hairc: [60, 54, 48], hair: 'styleRecede', cloth: 'dressshirt', c1: [150, 120, 86], tie: [120, 78, 52], glasses: true, mustache: true, brow: 'flat', mouth: 'neutral', heavy: true },
  'queue-ops': { skin: 'light', hairc: [154, 82, 46], hair: 'styleMessy', hairargs: { length: 15 }, cloth: 'blouse', c1: [176, 86, 74], brow: 'raised', mouth: 'smile', lashes: true },
};
...
function drawHeadGroup(buf: Buf, r: Recipe): void {
  ...
  if (r.mustache) drawMustache(buf, r.hairc);
...
// ─── public render ───────────────────────────────────────────────────────────
const bufCache = new Map<AgentCharacter, Buf>();
const sceneCache = new Map<AgentCharacter, SceneFrames>();

function getBuf(name: AgentCharacter): Buf {
  let buf = bufCache.get(name);
  if (!buf) {
    buf = compose(RECIPES[name]);
...
export function sceneFrameBufs(name: AgentCharacter): SceneFrames {
...
export function paintPortrait(ctx: CanvasRenderingContext2D, name: AgentCharacter, scale = 2): void {
